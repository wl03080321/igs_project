    ...
    try:
        if topic_id:
            await bot.send_message(
                chat_id=chat_id, text=message, message_thread_id=topic_id
            )
        ...
        logger.error(
            f"Failed to send to {chat_id}{' topic '+topic_id if topic_id else ''}: {e}"
        )
        return False
...
def get_telegram_topics(mongodb_client: MongoDBClient, group_id=None):
    try:
        ...
        topics = mongodb_client.query_by_fields(
            db_name, collection_name, filter_dict=filter_dict
        )
...
def get_weekly_report_data_by_category(mongodb_client: MongoDBClient, query_hours: int):
    try:
        ...
        # 先獲取所有類別
        all_categories = mongodb_client.get_distinct(
            db_name, collection_name, "category"
        )
